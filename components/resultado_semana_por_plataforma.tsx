import { fetchChoferSemanas, fetchPlataformaSemanas, fetchSemanasAndDay, fetchServiciosBySemanaChofer } from '@/lib/api_server'
...
export default async function RSemanaByPlataforma() {
    const result=await fetchPlataformaSemanas()
    //console.log(result)
                        ...
                        <TableBody>
                            {result&&result.map((d) => (
                                <TableRow key={uuidv4()}>
                                    ...
                                    <TableCell className="uppercase text-[12px]">{d.nombre}</TableCell>
