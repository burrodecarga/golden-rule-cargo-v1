import { fetchSemanasAndDay } from '@/lib/api_server'
...
export default async function RSemanaYDia() {
    ...
    if (result) {
        totalArray=result.map((a) => Number(a.precio_de_servicio))
                        ...
                        <TableBody>
                            {result&&result.map((d) => (
                                <TableRow key={uuidv4()}>
                                    ...
                                    <TableCell className="uppercase text-[12px]">{formatDateToLocal(d.fecha_carga!)}</TableCell>
                                    <TableCell className="uppercase text-[12px]">{WEEKDAY[d.dia_de_semana!]}</TableCell>
                                    <TableCell className="uppercase text-[12px]">{d.chofer}</TableCell>
                                    <TableCell className="uppercase text-[12px]">{d.precio_de_servicio}</TableCell>
                                    <TableCell className="uppercase text-[12px]">{d.gasto_estimado}</TableCell>
                                </TableRow>
                            ))}
                        </TableBody>
                        <TableFooter>
